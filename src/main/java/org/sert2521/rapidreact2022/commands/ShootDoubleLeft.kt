...
class ShootDoubleLeft : SequentialCommandGroup() {
    init {
        addCommands(
            ...
            DrivePath(AUTO_TURN_SPEED, AUTO_TURN_ACCELERATION, true, PICKUP_POSE_LEFT, ENTRY_POSE_LEFT).deadlineWith(IntakeBalls().withTimeout(0.5)),
            DrivePath(AUTO_SPEED, AUTO_ACCELERATION, false, ENTRY_POSE_LEFT, SHOOT_POSE).deadlineWith(RevShooter()),
